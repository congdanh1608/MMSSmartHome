...
//        listOfCommands.add("sudo sed -i.bak \"s|exec /usr/bin/X -nolisten tcp \"$@\"|exec /usr/bin/X -s 0 dpms -nolisten tcp \"$@\"|\" /etc/X11/xinit/xserverrc && sudo chmod 755 shell/*.sh RaspServer.jar && sudo mkdir -p /home/" + raspberryPi.getUsername() + "/.config/autostart && sudo cp start/startServer.desktop /home/" + raspberryPi.getUsername() + "/.config/autostart/ && sudo cp wifi_config/wpa_supplicant.conf /etc/wpa_supplicant/ && sudo cp /usr/share/zoneinfo/Asia/Ho_Chi_Minh /etc/localtime && sudo date -s \"" + UtilsMain.getCurrentTimeMMMddHHmmss() + "\" && echo EndCommands");
        listOfCommands.add("sudo chmod 755 deb/* start/* wifi_config/* wifi_router/* shell/* ic_homemms.png RaspServer.jar && sudo sed -i.bak s/POWERDOWN_TIME=30/POWERDOWN_TIME=0/g /etc/kbd/config && sudo sed -i.bak s/BLANK_TIME=30/BLANK_TIME=0/g /etc/kbd/config && sudo cp start/xorg.conf /etc/X11/xorg.conf && sudo mkdir -p /home/" + raspberryPi.getUsername() + "/.config/autostart && sudo cp start/startServer.desktop /home/" + raspberryPi.getUsername() + "/.config/autostart/ && sudo cp wifi_config/wpa_supplicant.conf /etc/wpa_supplicant/ && sudo cp /usr/share/zoneinfo/Asia/Ho_Chi_Minh /etc/localtime && sudo date -s \"" + UtilsMain.getCurrentTimeMMMddHHmmss() + "\" && echo EndCommands");
